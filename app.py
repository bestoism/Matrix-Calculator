...
def calculate_lu():
    try:
        ...
        # Diagonal matriks L adalah 1
        for i in range(n):
            L[i][i] = 1
        
        # Implementasi LU Decomposition
        for i in range(n):
            # Upper Triangular
            for k in range(i, n):
                sum = 0
...
def calculate_matrix_inverse():
    try:
        ...
        if abs(determinant) < 1e-10:  # Cek apakah matriks singular
...
def calculate_non_linear():
    try:
        ...
        x = symbols('x')
        f = sympify(equation)
        f_prime = diff(f, x)  # Derivative
        
        # Convert to lambda functions for numerical evaluation
...
def calculate_interpolation():
    try:
        ...
        if method == 'linear':
            ...
            # Polynomial interpolation using numpy's polyfit
...
def calculate_system_simulation():
    try:
        ...
        # Definisikan sistem persamaan Lotka-Volterra
        ...
        # Pisahkan solusi untuk prey dan predator
        prey = solution[:, 0]
        predator = solution[:, 1]
...
def calculate_monte_carlo():
    try:
        ...
        # Calculate points for curve plotting
        x_plot = np.linspace(x_min, x_max, 200)
        y_plot = [f_lambda(x) for x in x_plot]
        
        # Count points and calculate area
        points_under_positive = []  # Points under curve in positive region
        points_over_positive = []   # Points over curve in positive region
        points_under_negative = []  # Points under curve in negative region
        points_over_negative = []   # Points over curve in negative region
        
        points_contributing = 0  # Points contributing to area
...
def calculate_markov_chain():
    try:
        ...
        # Hitung probabilitas untuk setiap langkah
        ...
        # Hitung steady state (keadaan setimbang)
        eigenvalues, eigenvectors = np.linalg.eig(transition_matrix.T)
        steady_state_idx = np.argmin(np.abs(eigenvalues - 1.0))
        steady_state = eigenvectors[:, steady_state_idx].real
        steady_state = steady_state / np.sum(steady_state)  # Normalisasi
        
        # Generate labels untuk state
